...
@Composable
fun ComposablePopularScreen(viewModel: MovieViewModel, navController: NavController){
    val movieListDB by viewModel.popularMovieList.observeAsState(emptyList())
    LaunchedEffect(Unit) {
    ...
    if (movieListDB.isEmpty()) {
        ...
        Column() {
            Text("List movie where rate more than 0.5")
            LazyColumn() {
